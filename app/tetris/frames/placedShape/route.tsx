...
import {setGrid,updateShapes,levelHandler, getGameState} from '../../util/getGame'

const shapes = [ { "shape": [ [ 1 ] ], "color": "#01bfac" }, { "shape": [ [ 1 ], [  1 ], [  1 ] ], "color": "#84f4af" }, { "shape": [ [ 1 ] ], "color": "#c7f29d" } ]
const pointsPerCell = 5;
export const POST = frames(async (ctx) => {
  const currentState = ctx.state;
  const y = Math.floor((ctx.message.inputText - 1) / 10); // Row index based on the cell number
  const x = (ctx.message.inputText - 1) % 10;
  const newGrid = placeShape(currentState.shapes[currentState.activeShape].shape, currentState.grid, x, y)
  console.log('placing shape', currentState.shapes[currentState.activeShape].shape)

  const numCells = 
    currentState.shapes[currentState.activeShape].shape.reduce((count, row) => {
  ...
  let newShapes = currentState.shapes.filter(
    (_, index) => index !== currentState.activeShape
  );
